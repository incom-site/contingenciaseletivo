...
class GoogleSheetsService {
  ...
  constructor() {
    this.scriptUrl = import.meta.env.VITE_GOOGLE_SCRIPT_URL || 'https://script.google.com/macros/s/AKfycbxyU62K8MHdXsl1AD50TTtxBP27cs3wzSwyClo9N_uEe39PXiYEdu3IwzcicUEeApP8qQ/exec';
  }
...
export const candidateService = {
  ...
  async getCargosAdmin(): Promise<string[]> {
    try {
      const allData = await sheetsService.getCandidates();
      const uniqueCargos = [...new Set(allData.map(c => c.CARGOADMIN))];
      return uniqueCargos.filter(cargo => cargo && cargo.trim() !== '');
    } catch (error) {
      console.error('Erro ao buscar cargos admin:', error);
      throw error;
    }
  },

  async getCargosAssis(): Promise<string[]> {
    try {
      const allData = await sheetsService.getCandidates();
      const uniqueCargos = [...new Set(allData.map(c => c.CARGOASSIS))];
      return uniqueCargos.filter(cargo => cargo && cargo.trim() !== '');
    } catch (error) {
      console.error('Erro ao buscar cargos assis:', error);
      throw error;
    }
  },
